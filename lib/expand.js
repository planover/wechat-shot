...
// 通用兜底短句：当某场景语料池被抽空时，从这里面取，避免"同一句话出现两次"
const GENERIC_FILLERS = [
  '说起来这个我也遇到过', '细节我还没细看', '这事儿确实挺离谱', '先码住回头细品',
  '有一说一', '感觉还行吧', '蹲个后续', '学到了', '笑死', '哈哈哈蹲一个',
  '我寻思也是', '不至于吧', '这波我站楼主', '评论区说的有道理', '看个热闹',
];

// 去重随机：从 arr 中挑一个尚未用过的元素；arr 抽空时回退到通用兜底池，仍空才允许重复（极端情况）
function pickUnique(rng, arr, used) {
  let avail = arr.filter((x) => !used.has(x));
  if (!avail.length) avail = GENERIC_FILLERS.filter((x) => !used.has(x));
  if (!avail.length) avail = arr;
  const choice = pick(rng, avail);
  used.add(choice);
  return choice;
}
...
const SCENES = {
  ...
  zhihu: {
    names: ['知乎答主', '杠精本精', '匿名用户', '高赞老哥', '路过群众', '相关专业人士', '吃瓜群众', '理性派'],
    open: [
      (t) => `刚看到个神回答，${t}`,
      (t) => `知乎热榜：${t}`,
      (t) => `有个问题挺有意思，${t}`,
...
function expandToChat(rawContent, opts = {}) {
  ...
  const used = new Set(); // 记录已用过的原始语料，避免同一对话里重复同一句话
  ...
  const emitShort = (who) => emit(who, pickUnique(rng, SHORTS, used));
  const emitImg = (who) => { if (used.has('__IMG__')) return; used.add('__IMG__'); lines.push(`**${who}**：[图片]`); };
  const emitVoice = (who) => { const v = 1 + Math.floor(rng() * 20); const key = '__VOICE__' + v; if (used.has(key)) return; used.add(key); lines.push(`**${who}**：[语音]${v}`); };

  // 开场
  emit(A, pickUnique(rng, scene.open, used)(effectiveTopic));

  // 反应轮（1~2 人）
  const reactors = shuffle(rng, others).slice(0, 1 + (chance(rng, 0.6) ? 1 : 0));
  for (const who of reactors) {
    emit(who, pickUnique(rng, scene.react, used));
  ...
  for (let i = 0; i < bodyRounds; i++) {
    ...
    const roll = rng();
    if (roll < 0.34) {
      emit(who, pickUnique(rng, scene.question, used));
    } else if (roll < 0.7) {
      emit(who, pickUnique(rng, scene.comment, used));
    } else if (roll < 0.82) {
      emitShort(who);
    } else if (roll < 0.9) {
      emitImg(who);
    ...
    if (chance(rng, 0.2)) {
      ...
      emit(who2, pickUnique(rng, scene.comment, used));
    }
    // 偶尔自然插入红包/转账（仅在合适场景，金额/备注去重避免重复）
    if (chance(rng, 0.12)) {
      const rp = pickUnique(rng, ['恭喜发财', '新年快乐', '请喝奶茶', '接好运', '群发福利'], used);
      lines.push(`**${pick(rng, others)}**：[红包]${rp}`);
    }
    if (chance(rng, 0.08)) {
      const amt = pickUnique(rng, ['8.88', '5.20', '66.66', '13.14', '1.00', '52.00'], used);
      const note = pickUnique(rng, ['收到', '奶茶钱', '饭费', '谢啦', '回礼'], used);
      lines.push(`**${pick(rng, others)}**：[转账]${amt}:${note}`);
    }
  }

  // 收尾
  emit(pick(rng, names), pickUnique(rng, scene.close, used));
  ...
  if (chance(rng, 0.35)) {
    ...
    lines.push('');
    lines.push(buildTimeNode(rng, hour));
    const lateLine = '话说回来，' + effectiveTopic + '这事儿还有后续';
    used.add(lateLine);
    emit(pick(rng, names), lateLine);
    emit(pick(rng, others), pickUnique(rng, scene.react, used));
